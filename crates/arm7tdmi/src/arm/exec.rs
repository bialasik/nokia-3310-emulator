    ...
    >(
        ...
        if rlist != 0 {
            if LOAD {
                ...
                for r in 0..16 {
                    if rlist.bit(r) {
                        // Przy user-bank transfer (STM {..}^) rejestr w liscie nalezy do
                        // banku USER, a base (np. sp) do banku biezacego trybu - to ROZNE
                        // rejestry. Specjalny przypadek "base w liscie" (old_base) NIE moze
                        // sie wtedy stosowac: zapisalby base biezacego trybu w slot rejestru
                        // user-banku (np. sp_fiq zamiast sp_usr). Po change_mode(->User)
                        // get_reg(r) zwraca juz wartosc user-banku - jej uzywamy.
                        let val = if r == REG_PC {
                            self.pc_arm() + 12
                        } else if user_bank_transfer || r != base_reg {
                            self.get_reg(r)
                        } else if first {
                            old_base
                        } else {
